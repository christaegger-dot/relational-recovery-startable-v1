...
- **Touch-Targets**: Mindestens 44×44 px für alle interaktiven Elemente.
  - **Dokumentierte Ausnahme (Audit-09-Ausnahmemechanismus)**: `.ui-chip` in `primitives.css` setzt `min-height: calc(var(--touch-target-min) - 0.5rem)` = 36 px. Begründung: Filter-Chips im Netzwerk-Tab und Evidenz-Anchors erscheinen in dichten Reihen, in denen 44 px die Lesbarkeit zerschneiden würden. Die horizontale Touch-Fläche bleibt durch Padding und `gap` > 44 px, sodass Daumen-Treffgenauigkeit nicht leidet. Neue Komponenten sollten diese Ausnahme nicht ohne explizite Diskussion übernehmen.
